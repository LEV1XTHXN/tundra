/**
 * The dense, sortable spreadsheet-style List view of a folder — extracted
 * unchanged (same column resize/virtualizer/property-cell-editing behavior)
 * from what used to be the whole of `FolderTableView.tsx`, now just one of
 * its two body renderers (`FolderTableView.tsx` owns the shared header +
 * Gallery/List switch; see `FolderGallery.tsx` for the other one).
 */
                ...
                >
                  <div className="ft-td ft-td-name" style={colStyle("name")}>
                    <button
                      ...
                      onClick={() => (row.kind === "folder" ? onOpenFolder(row.path) : onOpenNote(row.summary.id))}
                    >
                      {row.kind === "folder" ? (
