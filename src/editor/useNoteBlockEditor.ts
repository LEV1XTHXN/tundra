...
export function useNoteBlockEditor({ note, vaultPath }: Params) {
  // BlockNote's own menu/toolbar strings, localized separately from our
  // i18next strings (see `src/i18n/blockNoteDictionary.ts`). Passed as an
  // explicit dep so the editor recreates — picking up the new dictionary —
  // when it finishes loading or the language changes, without needing a full
  // per-note remount.
